...
export function AccountChips({ accounts, value, onChange, showNone, disabledId, onAddAccount }: AccountChipsProps) {
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
      ...
      {/* Appended after every real account (not just the empty-state fallback above) — opens Add
          Account without leaving this form; found via on-device review that reaching it otherwise
          meant abandoning whatever you were already filling in. */}
      <Pressable onPress={onAddAccount} className="items-center gap-1 w-[56px]" accessibilityLabel="Add account">
        <View
        ...
        <Text className="text-[8px] font-medium text-center leading-tight text-tertiary w-full">Add</Text>
      </Pressable>
    </ScrollView>
  );
}
