...
/**
 * RN port of apps/web-legacy/src/features/import/PreviewStep.tsx. Uses `theme.success`/`theme.warning`
 * (real hex values) instead of web's `STATUS` CSS-var-string constant — the same fix already applied
 * everywhere else in this migration since `STATUS.x` silently fails as an RN style value.
 */
export function PreviewStep({
  ...
  return (
    <>
      ...
      <ListContainer>
        {preview.map((row, i) => (
          <View key={i} className="px-4 py-3 flex-row items-start gap-3" style={{ opacity: row.duplicate ? 0.45 : 1 }}>
            <View
