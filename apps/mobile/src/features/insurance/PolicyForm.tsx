...
/**
 * RN port note: web's `grid grid-cols-4` for the policy-type picker has no RN/NativeWind equivalent
 * (Yoga has no CSS Grid) — each tile is wrapped in a `w-[23%]` View inside a `flex-row flex-wrap`
 * container instead, same "4 columns with a small gap" result. Same swap for the 2-column amount row
 * (`grid-cols-2` → `flex-row gap-3` with `flex-1` children).
 */
export function PolicyForm({ editing, onSave, onDelete, onClose }: Props) {
  ...
  return (
    ...
    >
      <View>
        <Text className="text-xs font-medium text-secondary">Policy type</Text>
