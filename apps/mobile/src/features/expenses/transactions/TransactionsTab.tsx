    ...
    <View>
      {grouped.map((group, gi) => (
        <View key={group.label}>
          ...
          <View className="relative pl-10 pr-4 pt-4 pb-1.5">
            <View
              className="absolute w-px"
              style={{ left: 20, top: gi === 0 ? '55%' : 0, bottom: 0, backgroundColor: theme.border }}
            />
            <Text className="text-[11px] font-semibold uppercase tracking-wider text-tertiary">{group.label}</Text>
          </View>
            ...
            const icon =
              txnType === 'income'
                ...
                : txnType === 'transfer'
                  ...
                  : (cat?.icon ?? 'ti-dots');
            const amountColor =
              txnType === 'income' ? theme.success : txnType === 'expense' ? theme.danger : theme.info;
