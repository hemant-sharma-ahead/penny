  ...
  return (
    <Modal onClose={onClose} title="Share with a group">
      <Text className="text-[13px] text-secondary -mt-1">
        Adds <Text className="text-primary font-bold">{expense.description}</Text> ({formatCurrency(expense.amount)}) as
        an equal split. Your account still records the full amount.
      </Text>
