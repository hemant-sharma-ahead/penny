...
export function ChangePassphrasePage() {
  ...
  const canSubmit = current.length > 0 && score >= 3 && next === confirm && current !== next && !saving;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (result === 'ok') {
        ...
        showToast({
          message: 'Passphrase changed. Keep it safe — there is no way to recover it if lost.',
          variant: 'success',
          durationMs: 3000
        });
