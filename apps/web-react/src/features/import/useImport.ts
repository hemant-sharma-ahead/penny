 ...
 * `review` step (2026-07-28 redesign, see docs/mockups/proposals/import-wizard-redesign-v4.html): the
 * user adjusts category/account resolutions while looking at a LIVE preview, instead of confirming
 * once and only then seeing the result. This means the preview (`preview`, `rowTriage`, per-account/
 * per-category stats) is now computed reactively via useMemo off `categoryResolutions` /
 * `accountResolutions` / `parsedRows` — recomputed on every resolution change — rather than built once
 * by an explicit "confirm" action. New categories/accounts are NOT created in the DB on every keystroke:
 * that write only happens in `commitAndImport()`, the single final action, immediately before the
 * batch is written. File reading stays in the UI (platform glue); this hook works purely from
 * already-decoded text, same as before.
 */
export function useImport() {
  ...
  function importFromText(text: string) {
    ...
    // its Date/Amount/Description inputs from `mapping`, and previously it stayed null for every known
    // format, so "needs attention" rows rendered blank instead of the raw source data (found 2026-07-28).
  ...
  /** ALL detected pairs, for display — a pair involving a duplicate/skipped leg used to be filtered
   *  out of the "Linked transfers" list entirely (e.g. on a re-uploaded file whose rows were already
   *  imported once), vanishing with zero explanation even though the duplicate count elsewhere on
   *  screen still reflected them. Now every detected pair is shown, just marked. */
   ...
   *  Two source account names sharing the exact same suggested name+type (e.g. via a "same account,
   *  written two ways?" merge) create ONE account, not two. Also: no two accounts may ever share the
   *  same (trimmed, case-insensitive) name app-wide — if a 'create' suggestion's name exactly matches
   *  an account that already exists (e.g. the user typed over the New-account-name field, or ignored a
   *  fuzzy-match suggestion but happened to type the exact existing name), it's treated the same as an
   *  exact match elsewhere in this wizard: reuse the existing account instead of creating a duplicate. */
  async function commitAndImport() {
    ...
    // Redundant carry-forward rows (every occurrence but the earliest per account, see
    // importCarryForward.ts) must never be written — tracked here by object reference (not index) since
    // applyConfirmedTransferPairs below reorders the array (merged pairs first, then passthrough rows);
    // it preserves object identity for every row it doesn't collapse, and a carry-forward row is never
    // part of a confirmed transfer pair (isLikelyCarryForward rows are never 'transfer'-typed), so
    // reference equality survives the merge intact.
    const carryForwardExcludedRowRefs = new Set(finalRows.filter((_, i) => carryForwardExcludedIndices.has(i)));
    // Collapse each CONFIRMED transfer pair (both legs ready, computed above) into ONE merged
    // transfer row before writing — see applyConfirmedTransferPairs' doc comment. `confirmedTransferPairs`
    // was computed from the live preview, but its outgoingIndex/incomingIndex are stable indices into
    // `parsedRows` (unchanged since), so it's safe to reuse directly against `finalRows`.
