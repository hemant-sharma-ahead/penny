...
/**
 * RN port of core/nps/npsClient.ts. Web's `getNpsSchemes` caches the scheme list in `localStorage`
 * (doesn't exist on RN) on top of an in-memory cache. Per the same decision as `ipoClient.native.ts`,
 * mobile drops the persistent layer and keeps only the in-memory `schemesMemCache` — the scheme list
 * re-fetches once per cold app start instead of once per week.
 */
