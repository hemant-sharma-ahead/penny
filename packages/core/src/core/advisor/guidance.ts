...
/** Semantic app-screen keys a `navigate` action can point to. The app layer maps these to an actual
 * route/screen (a URL path on web, a navigator screen name on React Native) — core never knows about
 * routing. */
export type AppRouteKey = 'goals' | 'insurance' | 'expenses' | 'loans' | 'portfolio';
...
export type GuidanceAction =
  ...
  | { kind: 'navigate'; label: string; to: AppRouteKey }
...
export function guidanceForComponent(c: ScoreComponent, ctx: AdvisorContext): GuidanceAction | null {
  switch (c.key) {
    case 'emergency_fund': {
      ...
      if (ctx.hasEmergencyGoal) return { kind: 'navigate', label: 'Top up', to: 'goals' };
    ...
    case 'insurance':
      return { kind: 'navigate', label: 'Add cover', to: 'insurance' };
    case 'savings_rate':
      return ctx.incomeNeeded
        ? { kind: 'add-data', label: 'Add income' }
        : { kind: 'navigate', label: 'Review spend', to: 'expenses' };
    case 'debt_income':
      return { kind: 'navigate', label: 'Review', to: 'loans' };
    case 'goals':
      return { kind: 'navigate', label: 'Review', to: 'goals' };
    case 'diversification':
      return { kind: 'navigate', label: 'Invest', to: 'portfolio' };
    default:
      return null;
  }
}
