...
// as `AutoBackupCard` renders and calls `isAvailable()`). Otherwise this is the same Google Identity
// Services + Drive v3 REST flow as the base file — RN Web is a real browser DOM (script tags,
// `window.google`, `fetch`, `FormData`, `Blob` all work exactly the same), only the config source
// differs: `app.json`'s `extra.googleWebClientId` (same key `googleDriveProvider.native.ts` reads),
// not a `VITE_*` env var.
