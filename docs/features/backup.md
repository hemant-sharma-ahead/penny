...
**Mobile (`apps/mobile`):** ported alongside the rest of Track 4's remaining-modules pass — `apps/mobile/src/features/backup/BackupPage.tsx` + `AutoBackupCard.tsx` mirror the web files above 1:1. Platform notes: web's object-URL `<a download>` export becomes `expo-file-system` (write to cache) + `expo-sharing`'s native share sheet (same pattern as CSV export); restore's file picker uses `expo-document-picker` + `File.text()`; post-import/-reset, web's full page reload (so its router re-evaluates) becomes `notifyAuthShouldRecheck()` (same precedent as `ChangePinPage`'s `'wiped'` case). The automatic-backup engine itself was upgraded to run natively: `packages/core/src/core/sync/SyncProvider.native.tsx` re-runs the engine on `AppState` returning to `'active'` instead of web's `online`/`visibilitychange` DOM events, and `backupPrefs.native.ts` (previously in-memory-only, added ahead of this pass for `ProfilePage`'s backup-nudge banner) now hydrates from/writes through to AsyncStorage. Google Drive stays unreachable on mobile (`isCloudBackupConfigured()` false — needs a native Google Sign-In flow, a real future track) and the on-device OPFS floor is a no-op on RN (`isLocalBackupAvailable()` checks a browser-only API) — both degrade to the same "not configured" UI web already shows when unset, so nothing needed to change to avoid crashing.
