...
- **Safe to spend** — a single liquidity-based number: current liquid balance − committed outflows until your next payday − your safety cushion − **money already saved toward your goals** (2026-08-02). Framed by payday ("to last the next 8 days till payday") with a per-day allowance. Falls back to month-end when no payday is forecast. Tapping the "Excludes ₹X saved for goals" line expands a per-goal breakdown.
- **Goal exclusion** (2026-08-02) — every Goal has a "Counts toward Safe to spend" toggle (`GoalForm`, default **on**). A goal's saved amount (baseline + contributions, same figure the Goals module itself shows) is subtracted from the liquid balance before Safe-to-spend is computed, since that money physically sits in the same account as the rest of your cash with no separate wallet — otherwise it reads as spendable twice. Defaulting every goal to "on" is deliberate: a Goal only exists because you deliberately set money aside for something, so there's no natural goal category that should default to being freely spendable — the toggle is a manual escape hatch for whichever specific goal you personally still want to treat as spendable (e.g. a small near-term purchase fund), not an automatic "type" inference.
...
- **In-app reminders** — a header bell + badge surfaces overdue recurring bills and anything due in the next 7 days (EMIs, subscriptions, insurance, bills) with snooze / mark-done / log / cancel / **mark as paid** (2026-08-31, Term/Life insurance premiums specifically — calls the shared `apps/mobile/src/hooks/useInsurancePremiumActions.ts` mutation) actions. Built from the same forecast + recurring-due data (`core/reminders`, `hooks/useReminders`). In-app only; OS/push reminders are a Phase 2 item.

## How it works

**Pure engine — `src/core/cashflow/forecaster.ts`:**

1. **`forecastEvents`** projects dated events across the horizon, emitting **every occurrence** that falls within it (so a 3-month view shows monthly rent/EMI/subscriptions in each month): loan EMIs (monthly on `emiDueDate` until `endDate`), subscriptions (from `lastChargedAt` stepping `intervalDays`, amount from `detectedAmount`, labelled by `merchantCategory`), insurance (see below), recurring expenses (out) and **recurring income** (in).
   - **Insurance (2026-08-31 redesign):** a Term/Life policy with a real payment schedule set (`paymentFrequency !== 'S'` and `nextPremiumDueDate` defined) emits one event per real due-schedule occurrence within the horizon (`core/insurance/premiumSchedule.ts`'s `scheduledOccurrencesWithin`, discount-aware `installmentAmount`), each carrying a `policyId` — instead of one flat annual guess. Every other case (Health/Vehicle/Home/Travel/Other, or a legacy/Single-premium Term/Life record with no schedule) keeps the original flat annual event from `renewalDate`/`annualPremium`. A Term/Life policy that's genuinely paid up (`isPaidUp()` — a Limited Pay policy past its pay term) emits **no** insurance event at all. See `docs/features/insurance.md`. **Only CONFIRMED subscriptions drive the projection** — the forecaster skips any subscription where `!confirmedByUser` or `status === 'cancelled'`. Detected-but-unconfirmed candidates are hidden from the Subscriptions tab, so they deliberately do **not** appear as cash-flow outflows (otherwise Cash Flow would show a payment the user can't see or manage). Each event carries a `direction` (`'in' | 'out'`). Transfers are skipped (net-zero across own liquid accounts). A recurring series is logged every period, so each (type + merchant) is **collapsed to its most recent occurrence** before projecting — otherwise every historical row would project into duplicate events, which means **recurring series must share a stable description** across months (the demo seed does this; per-month suffixes would break it). A charge tracked _both_ as a confirmed subscription and as recurring expenses would be counted twice, so the two shouldn't overlap (the demo seed keeps subscription-backed expense rows non-recurring).

2. **`projectBalance`** starts from the current liquid balance and walks day-by-day to the horizon end, accumulating signed deltas into a running-balance series. It derives the **lowest** point, the first **buffer-breach** day, total in/out, **net flow**, the **next payday** (`nextIncomeMs`), and a liquidity-based **`discretionary`**: `currentBalance − committedOutflowsUntilPayday − buffer`. `projectBalance` itself stays goal-agnostic (pure balance/commitments/buffer math).
...
**Goal-aware Safe to spend — `hooks/useForecast.ts`:** loads goals + `GoalContribution`s independently (this hook lives in `~/hooks/`, so it can't depend on `features/goals/useGoals.ts`) and computes each goal's saved amount via the shared `core/goals/progress.ts` (`effectiveSaved`/`goalReservations`/`totalGoalReserved` — the exact same math `useGoals.ts` uses for its own progress bars, not a second copy). `safeToSpend = max(0, discretionary − totalGoalReserved)`, exposed alongside the raw `discretionary` and a `goalBreakdown` (every goal + whether it counts) for the expandable UI. All three "Safe to spend" surfaces (Home, Expenses, Cash Flow) read `safeToSpend`, not `forecast.discretionary` directly.
