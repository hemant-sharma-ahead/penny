...
- Description is the first field in the Add form; as you type, Penny shows ranked type-ahead suggestions of remembered merchants beneath it (substring match). The whole suggestion row is tappable (not just a small trailing button) and fills the merchant, category, account, payment mode, and — when known — the amount from that merchant's most recent matching transaction (`MerchantMemory.amount`, still editable afterward); the row shows that amount instead of a plain "Use" label. Nothing changes until you tap. A merchant you've logged under more than one category surfaces as a separate suggestion per category, ranked by how often you use each.
...
- Import transactions from a Penny CSV template (round-trips its own export, including the Account/IOU
  Person/Shared-to-Group columns below), Cashew export, MoneyView export, or **any arbitrary CSV**
  (Custom, with a smart pre-filled column-mapping guess, not a blank one). **YNAB support was removed
  2026-08-23** (real-device-testing-pass item 78) — never reached the confirmed-adoption bar the other
  three formats did; Custom already covers the same ground for anyone with a YNAB export. **Redesigned
  2026-08-14** (`apps/mobile` only — `apps/web-react` is frozen and keeps the older single-review-screen flow described in its own history) into a 6-step wizard: **Upload → Map Columns (Custom only) → Accounts → Categories → Transactions → Done** — each of the three resolution stages is now its own screen instead of one merged review page, and nothing is written to the vault until a single final commit at the end of Transactions. Full design rationale, every decision considered and rejected, and the complete list of bugs found via real on-device testing: [`docs/plans/csv-expense-import-redesign.md`](../plans/csv-expense-import-redesign.md).
  - **Accounts stage**: one row per distinct source account name — existing/create/**skip** (skipping excludes every one of that account's rows from the rest of the import entirely, so they never need a category resolved), plus a **card→account merge suggestion** (a debit/credit-card-type row sharing a bank name with another resolution gets offered a merge into that account, with payment mode set from the card instead of tracking the card as its own account) alongside the existing fuzzy "same account, written two ways?" merge suggestion. Rows bucket into **Needs Review / Ready / Skipped** (an auto-suggested `create` must be explicitly touched/acknowledged before it counts as ready) and the stage blocks advancing until every account is genuinely resolved — deliberately stricter than Categories below.
  - **Categories stage** (new): one row per distinct source category **and direction** (an expense-direction and income-direction split of the same source name get fully independent resolutions — fixes a real bug where categorizing one silently re-categorized the other). Any category flagged transfer-suspect or lend/borrow-suspect (keyword-matched, e.g. "A/c to A/c", "Loan") is further split by counterparty — tiered by confidence: a normalized match against an existing IOU `Person` record, an editable low-confidence text candidate, or a residual "no clear person" group — since a single moneyview-style label can conflate self-transfers, lending, and repayments under one name. A category flagged as an investment movement (e.g. "Investments", "Mutual Funds") defaults to transfer-suspect/needs-review rather than silently becoming a plain expense category. Rows bucket the same Needs-Review/Ready/Skipped way as Accounts, but **advancing to Transactions is never blocked** — an unresolved category just flows through as an ordinary "needs attention" item, exactly like Transactions already had to handle unconfirmed guesses.
  - **Transactions stage**: same three-bucket review model as before (Needs your input / Staged — ready to import / Already imported), plus a real **Skipped** bucket (previously computed but never surfaced) and a capped, internally-scrolling row list per tile (year-inclusive dates) in place of the old unbounded "+N more" expand. **2026-08-19:** that per-tile row cap didn't cap the number of *tiles* itself — each of the three buckets rendered its tiles via a plain unbounded `.map()`, a real gap against the project's own bulk-import render-cap rule for a CSV that groups more granularly than a bank statement. Added the same "Show N more" pattern to all three bucket lists (cap 25 tiles, smaller than the row cap since each tile is a much heavier component); `review/CarryForwardExcluded.tsx`'s previously-uncapped row list got the same treatment (cap 60, matching the row-list convention). Each tile's full existing/create/transfer/skip picker stays reachable as a standing per-tile override — pre-populated with whatever the Categories stage already decided, not blank — and a direct **Skip** button now sits in the tile footer for a one-tap group skip without opening that picker at all. Structurally-unparseable rows get a **Discard** action alongside "Include this row" (silently excluded before this, with no way to dismiss them). "Already imported" rows can be un-flagged ("not a duplicate — import anyway"). Picking one of the four IOU categories opens the same lock-open Lent/Borrowed panel bank-import's bulk-categorize modal already has, pre-filling the person from whatever counterparty the Categories stage detected. **Import is genuinely partial**: the commit button is disabled only when nothing at all is ready — never because something still needs review — and only ready/skipped rows are actually written; unresolved ones are simply left for a later re-upload of the same file, picked up automatically by the exact-match dedup (which itself was fixed from a real day-truncated key that falsely flagged distinct same-day transactions as duplicates within a single file).
  - **Import Progress screen** (replaces a plain in-button spinner with no way to prevent leaving mid-write): tapping Import shows a pre-start summary (row count, estimated duration, a "Start Import" button — nothing written yet, back navigation still allowed) → tapping Start begins the real write with live progress/ETA and a Cancel button (confirms before stopping; whatever already wrote stays, the rest waits for a later re-upload) → a completion screen distinguishing a full finish from a stopped-early one. Back navigation (header, Android hardware back, and the swipe-back gesture) is fully locked only once writing has actually started, and is guaranteed to release via a `try/catch/finally` around the whole commit — found and fixed as a severe bug during this redesign: without it, any unexpected exception mid-commit left the screen permanently stuck with no way to leave.
  - Self-account movement (cash withdrawal, wallet top-up, credit-card bill payment — money moving between two of the user's own accounts) is detected by one general mechanism, not three separate heuristics. Structurally-unparseable rows (missing date/amount/description), collapsed **linked transfer pairs**, and MoneyView-style monthly **carry-forward markers** (see the plan doc for the full mechanics of each) all still work as before. No two accounts may ever share the same name, whether created via import or the manual Add Account form.
- **SMS-Based Transaction Tracking** (Android only, off by default) — a third way to record a transaction, alongside manual entry and CSV import: reads bank SMS and turns them into draft transactions, linking to an already-recorded transaction rather than duplicating it. Full detail: [`docs/features/sms-tracking.md`](sms-tracking.md).
...
The category system has three levels: intentGroup (parent group), ExpenseCategory (child category), and hashtags (free-form tags). Default categories are seeded from `defaultCategories.ts` at first run. A **Legal** intent group (Advocate Fee, Court Fee, Stamp Duty, Notary, Filing & Documentation, Affidavit, Typing & Printing, Exemption Fee, Legal Transport & Hotels, Legal Food & Refreshments, Other Legal Fees) ships as defaults and is back-filled to existing users via the additive `penny_cats_v4` seed in `useExpenses`.
...
**Income section: fixed mis-grouping + a cross-side drill-down leak (2026-08-19).** Some income-type
transactions carry an expense-only category left over from before the category picker enforced
`applicableTo` (e.g. `Loan EMI`, `Savings Transfer`, `Insurance Premium`). `buildIncomeData` grouped by
that category's raw `groupKey()`, so those rows rendered under a "Financial" sub-group — the exact
string `buildSetAsideData` already uses for its own unrelated expense-side row — which both mislabeled
the income row and collided `expandedGroup` state between the two (expanding one expanded the other).
Separately, the shared `viewGroup`/`viewCategory` drill-down callbacks had no `e.type` check at all, so
opening "View all transactions" from either side could leak the other side's transactions into the
popup. Fixed with a new `incomeGroupKey()` (`useExpenseAnalytics.ts`) — a category only earns its own
top-level row if it's itself `applicableTo: 'income'`, everything else folds into a fixed `'income'`
bucket (still fully visible in the category list, just without its own row) — plus new
`viewIncomeGroup`/`viewIncomeCategory` (`AnalyticsSlice.tsx`), explicitly `e.type === 'income'`-scoped
and wired only to `IncomeSection`; `viewGroup`/`viewCategory` now explicitly require
`e.type === 'expense'` too, closing the leak in both directions.

**Income + Total Spent moved above the ring graph (2026-08-19).** Per explicit request, in all three
views (Monthly/Annual/All-time) `IncomeSection` now leads the whole Analytics tab, ahead of even
`PulseCard` ("Total spent"), which itself still renders before `DailyLivingCard` (the ring). Render
order is now: Income → Total spent → ring → (annual-only: savings-rate headline, income-vs-spend
chart, biggest movers) → Events → Daily-routine spending → Set aside → hashtags → Cash Flow (still
last). Nothing else in the order moved.
...
**Three more real bugs found and fixed 2026-08-06, in this generic CSV/Cashew/YNAB/MoneyView (YNAB
since removed, see the 2026-08-23 review below) import flow specifically (distinct from Bank Statement Import — see `docs/features/bank-import.md`):**
...
'transfer'` row — and the same rows kept reappearing for manual categorization on every re-review. Root
cause took 3 fix rounds to fully close, worth remembering as a pattern: a fix applied only at
`commitAndImport()` (the final-write path) doesn't fix what the *live* `rowActions` memo shows the user
while still reviewing — that memo drives the categorization UI independently and can keep re-poisoning
the same rows even after the commit-time logic is correct. The actual fix applies
`releaseConfirmedPairsFromGroupSkip()` (`importPipeline.ts`) at both the live `rowActions` memo (keyed on
all `transferPairs`, immediately after the per-group loop and before the account-skip pass) and at
commit time. Also addressed: Cashew records a transfer as **two** separate rows under a "Balance
Correction" category (one debit, one credit) — Penny's model has no such split, so
`applyConfirmedTransferPairs()` now merges each detected pair into exactly one transfer row on import
instead of importing both halves. `isLikelySelfAccountMovement()`/`isLikelyCashWithdrawal()`
(`importCategoryResolution.ts`) also improved category-defaulting for this case, though that alone
wasn't the actual root cause.

**MoneyView CSV import review, 2026-08-23** (real-device-testing-pass items 69/72/73/74, plus mockup-gated
70/71/75 — `docs/mockups/proposals/moneyview-import-review-v1.html`):

- A full-screen `PennyLoader` now covers the gap between picking a file and the Accounts stage actually
  appearing (`UploadStep.tsx`) — a large file used to freeze the UI with zero feedback while the parse +
  account-resolution scan ran synchronously.
- **Bank/card merge ambiguity**: `suggestCardAccountMerges` (`importAccountResolution.ts`) only offers a
  confident card→account merge when exactly one non-card resolution shares its bank key; a new
  `findAmbiguousCardAccountMerges` export (+ `CardAccountMergeAmbiguity` type) catches the 2+-same-bank
  case instead (e.g. two separate SBI accounts) and forces a manual pick in `AccountsSection.tsx` rather
  than a possibly-wrong confident guess.
- **Cash-withdrawal → transfer preview** now renders a `TransferPairCard`-style from→to card per accepted
  suggestion (source account → chosen cash account, inline per-row date+amount, "See all" beyond 4 rows
  opening `CashWithdrawalSeeAllModal.tsx`) instead of a bare one-line Undo banner. Follow-up fix: a
  category like "Cash Withdrawal" spanning several real bank accounts used to collapse into one vague
  "Multiple accounts" card — new `packages/core/src/core/import/importCashWithdrawalGrouping.ts`
  (`groupCashWithdrawalCandidates`) now partitions by resolved source account first, producing one
  accurate card per real account, each independently acceptable/dismissible/undoable via a compound key.
  Accepted rows surface in a synthetic "Bank Transfer" tile (`MovedRowsTile.tsx`), not the original
  category tile.
- **Carry-forward exclusion** (`importCarryForward.ts`'s `identifyRedundantCarryForwardRows`) no longer
  keeps the chronologically-earliest carry-forward row per account out of the excluded set — every
  carry-forward row is now excluded unconditionally, and `CarryForwardExcluded.tsx`'s caption was updated
  to match. Opening balance still stays `0` for accounts created this way, unchanged.
- **"Already Imported" duplicates bucket** (`DuplicatesBucket.tsx`) is now grouped by CSV category into
  collapsible tiles matching `CategoryTile`'s header style (the existing 60/60 render cap now applies per
  group, not once for the whole bucket), with a "See all N →" per group opening the new
  `DuplicatesSeeAllModal.tsx`. The paired CSV-row/matched-expense card was extracted into a shared
  `DuplicatePairRow.tsx` so it isn't duplicated between the inline list and the modal. `TileRowList.tsx`'s
  auto-collapse threshold is now exported as `INLINE_ROW_THRESHOLD` instead of an inline literal.
- Skipped-account summary on the Done screen now breaks down per account name (`useImport.ts`'s
  `accountSkippedBreakdown`, e.g. "142 transactions skipped — Freecharge (89), Paytm (53)") instead of one
  combined number.
- New regression coverage: `packages/core/tests/import/moneyviewTransferRegression.test.ts` (a synthetic
  MoneyView-shaped two-leg self-transfer collapses to one `type: 'transfer'` row).

**Penny CSV export/import round-trip + Custom-mapping field coverage review, 2026-08-23**
(real-device-testing-pass items 76-79 — mockup for 79: `docs/mockups/proposals/
penny-csv-mapping-and-group-leave-v1.html`):

- **Export gained 3 columns**: Account, IOU Person, Shared To Group, alongside the existing Date, Amount,
  Description, Category, Type, PaymentMode, Tags, Notes. `exportExpensesAsCsv` (`exportCsv.shared.ts`)
  takes an optional 3rd `context?: ExportCsvContext` (`{ accounts?, iouPersonByExpenseId?,
  groupNameById? }`); Shared To Group is explicitly informational ("Shared to: X"), not re-import-
  actionable as a real group link.
- **Import recognizes those 3 new Penny-format columns on re-import**: `ColumnMapping`/`ParsedRow` gained
  `iouPerson` and `sharedToGroupNote` (the latter folded straight into `notes`, appended not overwritten,
  never exposed as its own field). On commit, `useImport.ts` resolves a row's `iouPerson` into a real
  `Person` + `LedgerEntry` via the canonical `getOrCreatePerson` (replacing a pre-existing local ad-hoc
  person-resolver closure that had been missed by the earlier `personResolver.ts` consolidation) — it
  wins over the existing category-mandatory IOU flow when present.
- **YNAB format removed entirely** — dropped from `ImportFormat`, `IMPORT_FORMATS`, `FORMAT_LABELS`,
  `FORMAT_COLUMNS`, `FORMAT_SYNONYMS`, `FORMAT_DATE_HINT` (`importParsers.ts`). Never reached the
  confirmed-adoption bar the other three formats did; Custom already covers the same ground.
- **`MapColumnsStep.tsx` (Custom format) now surfaces every Penny-supported field**, not just the 6 it
  used to hardcode (Date, Description, Amount/split, Category, Account, Notes). Required + a "commonly
  used" optional tier (adds Payment mode) stay always-visible; the 4 rarer fields (Tags, a Transaction-type
  text/income-flag toggle, Bank name, Account type) live in a collapsible "More fields" disclosure —
  reusing `DuplicatesBucket.tsx`'s chevron-header pattern — that auto-expands with an "N auto-matched"
  badge whenever the initial guess pre-filled anything inside it, so a pre-filled value never hides
  unseen. Closes a real parity gap: nothing downstream of `ColumnMapping` branches on which format
...
- `src/core/import/importParsers.ts` — CSV tokenizer, the 4 format presets (Penny/Cashew/MoneyView/Custom — YNAB removed 2026-08-23) over importMatcher, rejected-row tracking
- `src/core/import/importCategoryResolution.ts` — per-distinct-source-category resolution (existing/transfer/create/skip), transfer-keyword + intent-group-keyword suggestion, `isLikelyCarryForward()` (carry-forward-keyword detection, kept separate from `isLikelyTransfer()`'s keyword list), `isLikelySelfAccountMovement()`/`isLikelyCashWithdrawal()` (category-defaulting for Cashew-style inter-account movement, see the Cashew fix above)
...
- `src/core/import/importPipeline.ts` — dedup keys (incl. in-batch), the resolution-based preview-row builder. **2026-08-31 fix:** a manually-entered expense carries the real wall-clock time it was saved at, while e.g. Cashew's CSV export is always midnight-local with no time-of-day, so an otherwise-identical (day/amount/description) manual duplicate never matched the exact-timestamp `dedupKey()`. Added `dedupDayKey()` (day-truncated), used **only** when matching an imported row against **existing DB expenses** (`buildResolvedPreviewRowsByIndex`'s `existingExpenseIdsByKey`) — in-batch/same-file matching stays on the exact `dedupKey()` unchanged, since that one needs to stay exact (see the note above about same-day collisions within one file). The existing "Not a duplicate — import anyway" per-row override is the safety valve for any resulting false positives. `applyConfirmedTransferPairs()` (collapses a confirmed transfer pair's two rows into one `toAccountId`-carrying row before write), `releaseConfirmedPairsFromGroupSkip()` (releases a group-skip that would otherwise re-poison an already-confirmed transfer pair's rows — applied at both the live `rowActions` memo and at commit time, see the Cashew fix above), plus a **legacy** section (`matchCategory`/`buildPreviewRows`) that predates the 2026-07-28 resolution-based rewrite — kept only so any old references still compile; no current caller on either platform uses it. `toConfirmedCategoryMap()` takes an optional per-source-category `tags: Map<sourceName, tag>` (from `CategoryTile`'s "Tag all transactions" field), normalises it (trim/strip leading `#`/lowercase, same as the manual hashtag input elsewhere), and `buildResolvedPreviewRows()` appends it onto each matching row's own `hashtags` (deduped, never overwritten) — reflected live in the preview and applied identically at `commitAndImport()`.
...
- Bank statement auto-import exists (see [`features/bank-import.md`](bank-import.md)) and SMS-based auto-tracking exists on Android (see [`features/sms-tracking.md`](sms-tracking.md), not yet verified on a real device) — this line previously claimed neither existed, which was stale even before SMS Tracking shipped
