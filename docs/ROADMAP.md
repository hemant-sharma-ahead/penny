...
**Last updated:** 2026-07-01 (Track E — Groups & Household OS, E1: `workers/groups/` ciphertext-only shared-ledger relay (D1+R2+KV, signed + membership/role-checked routes; device keys via read-only auth D1), `src/core/groups/` Group-Key gen + ECDH-wrapped grants + event enc/dec, `GROUPS_BASE`, Dexie v9; Model B)
...
| Phase 1.5        | Groups & Household OS — shared expenses, family vaults, joint goals, household net worth ([plan](plans/phase-1.5-groups-household-os.md)) | 🚧 In progress (Tracks 1 ✅, A API Proxy ✅ deployed, B client crypto ✅, C auth/identity ✅, D backup+sync ✅; Track E 🚧 — E1 ✅ worker + crypto; next E2) |
...
| **Groups** 🚧 E1      | Phase 1.5 Track E    | Group creation, member management, encrypted shared event ledger, key exchange + rotation. **E1 built (`workers/groups/` + `src/core/groups/`)** — ciphertext-only D1+R2 relay, signed + membership/role-checked routes, per-epoch Group Key with ECDH-wrapped grants, Dexie v9 mirrors. Group UX (create/invite/join/split/settle) in E2–E5                            |
