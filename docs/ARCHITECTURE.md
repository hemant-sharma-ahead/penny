...
| `apps/web-legacy/src/components/ui/`  | ✅ Done — rewritten as `apps/mobile/src/components/ui/` | Same prop APIs, different renderer (Track 3)                          |
| `packages/core/src/core/db/`          | ✅ Done — `expo-sqlite` adapter behind `RowStore<T>` | `EncryptedRepository<T>`'s constructor narrowed from Dexie's `Table` to `RowStore<T>` — type-only change on web |
