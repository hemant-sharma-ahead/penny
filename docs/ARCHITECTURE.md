...
**Verification tool:** `apps/mobile/src/screens/ComponentGalleryScreen.tsx` renders every ported component with representative props — currently wired as the reachable screen (since real onboarding UI doesn't exist until Track 4, `AuthGuard`'s `needs_onboarding` state has nothing else to show yet). Intended for the plan's "visual checklist per component against the 4 themes" verification step once a device/simulator is available; safe to delete once Track 4's real screens exercise every component anyway.
