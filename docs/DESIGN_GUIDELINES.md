...
- **Icon-tile selector** — a rounded icon **container** with the label **below / outside** it (never a caption crammed under a tiny icon). Selected = filled with `--color-primary`. This is the canonical multi-option selector (Settings modules, Employment).
