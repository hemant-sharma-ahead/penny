...
- **Privacy is the product, not a feature.** It's a lifestyle, not a checkbox — never something the user has to opt into or choose; it's the default, always. The user is always in control of their data and how it's used/shared, and privacy never comes at the cost of transparency — if something is masked, encrypted, or shared, the UI should make it legible *why* and *how*, not just *that*.
