...
**Current status:** see [`docs/MOBILE_PARITY.md`](../MOBILE_PARITY.md) for live per-module
parity status, and `CLAUDE.md`'s reference table for the one-line project status. This doc
is deliberately **not** a dated progress log anymore — it was one for a long time (2,500+
lines of session-by-session narrative), and that made "what's actually true right now"
something you had to excavate rather than look up. What follows is the durable
reference instead: why this migration exists, the tech stack actually chosen and why, the
technical requirements, and a distilled playbook of the concrete lessons already learned
the hard way — so the next porting or sweep pass doesn't rediscover them.
...
its `renderItem` in `useCallback` does nothing if the *props* passed into them (arrays,
objects, callbacks) get a new identity on every parent render. Trace the *whole* prop chain
...
or a hardcoded Tailwind class *looks* right at first glance but silently stops reacting to
...
Every feature folder in `apps/web-react/src/features/` has a corresponding, ported
counterpart in `apps/mobile/src/features/` (24/24 — see `docs/MOBILE_PARITY.md` for
within-module depth status, since folder-level coverage alone doesn't mean full parity).
Real onboarding (13 screens) sets a real Data Master Key on-device. IOU stays
...
minimal CI for `packages/core`/`apps/web-react` tests.

### Track 7 — Web cutover — not started
Once `apps/mobile`'s `react-native-web` build reaches documented parity with
`apps/web-react` (every `docs/features/*.md` capability, the `max-w-[430px]` layout,
cross-platform Drive backup, no functional regressions — tracked via
`docs/MOBILE_PARITY.md`), retire `apps/web-react` so exactly one codebase remains. This is
the long-term vision noted in `docs/ROADMAP.md`, inspired by looking at how mature
cross-platform apps (e.g. Cashew, a Flutter app with one codebase for every platform)
avoid a permanent two-codebase structure — Penny can't do that today because React DOM and
React Native are different renderers, but `react-native-web` is the actual path to the same
end state once mobile is fully verified.
