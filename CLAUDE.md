...
| **Mobile Migration — React Native (Expo)** — [plan](docs/plans/mobile-migration.md)    | 🚧 In progress, on branch `feat/rn-migration`. **Track 0 ✅ + Track 1 ✅** committed. **Track 2 ✅** storage + crypto adapter swap (`expo-sqlite`, `react-native-quick-crypto`, `AuthGuard` now calls real `@penny/core` functions) — verified via real bundle inspection + a crypto test-vector suite, but **on-device verification (real quick-crypto runtime, PBKDF2 timing) is still owed** (headless dev environment) — not yet committed. **Track 3 (core UI component library port) = next.** Supersedes the "native apps" line item below — see the plan for locked decisions (single Expo codebase incl. web via `react-native-web`, NativeWind, `expo-sqlite`+`react-native-quick-crypto`). |
