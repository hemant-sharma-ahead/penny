...
| **Mobile Migration — React Native (Expo)** — [plan](docs/plans/mobile-migration.md)    | 🚧 In progress, on branch `feat/rn-migration`. **Track 0 ✅** repo restructuring (pnpm workspace, `packages/core`/`apps/web-legacy` split) — committed. **Track 1 ✅** Expo app skeleton (`apps/mobile`: NativeWind dynamic theming, React Navigation shell, stubbed AuthGuard, `docs/RUNNING_MOBILE.md`) — verified (web export, `tsc -b`, eslint, full test suite) but **not yet committed**; on-device/simulator check still owed (headless dev environment). **Track 2 (storage + crypto adapter swap) = next.** Supersedes the "native apps" line item below — see the plan for locked decisions (single Expo codebase incl. web via `react-native-web`, NativeWind, `expo-sqlite`+`react-native-quick-crypto`). |
